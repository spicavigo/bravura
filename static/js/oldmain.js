...
function plot(name, opt){
    ...
    opt = jQuery.extend(opt, {
                    ...
                    mouse: {
		                ...
		                trackDecimals: 2,
		                trackFormatter: function(obj){ cosole.log(1);return 'x = ' + obj.x +', y = ' + obj.y; }
...
function remove_series(name, series){
    ViewableSeries[name][series] = 0;
    plot(name);
}
function add_series(name, series){
    ViewableSeries[name][series] = 1;
    plot(name);
}
function get_var(name, type, options){
    type = type || 'get';
    options = options || {};
    options['type'] = type;
    options['name'] = name;
...
function add_items(){
    for (var i=0; i<PARAMS.length; i++){
        ...
        jQuery('#container').append('<div class="item">' + buttonHTML + '<div class="graph" id="'+PARAMS[i] + '"></div>');
        //get_var(PARAMS[i]);
    ...
    jQuery('#container').delegate( '.item .togglesize', 'click', function(){
